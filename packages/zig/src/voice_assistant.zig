...
pub const IntentParameter = struct {
    ...
    display_name_buffer: [128]u8 = @splat(0),
    ...
    default_value_buffer: [256]u8 = @splat(0),
    default_value_len: usize = 0,

    /// Prompt to ask user for this parameter
    prompt_buffer: [256]u8 = @splat(0),
    prompt_len: usize = 0,
...
pub const IntentDefinition = struct {
    ...
    title_buffer: [128]u8 = @splat(0),
    title_len: usize = 0,

    /// Description for users
    description_buffer: [512]u8 = @splat(0),
    ...
    invocation_phrases: [8][256]u8 = @splat(@splat(0)),
    invocation_phrase_lens: [8]usize = @splat(0),
    invocation_phrase_count: usize = 0,

    /// Parameters (max 8)
    parameters: [8]IntentParameter = @splat(IntentParameter{}),
...
pub const ParameterValue = struct {
    ...
    /// String value
    string_value_buffer: [512]u8 = @splat(0),
...
pub const VoiceRequest = struct {
    /// Request ID
    request_id_buffer: [64]u8 = @splat(0),
    request_id_len: usize = 0,

    /// Intent ID
    intent_id_buffer: [128]u8 = @splat(0),
    intent_id_len: usize = 0,

    /// Raw transcription
    transcription_buffer: [1024]u8 = @splat(0),
    ...
    parameters: [8]ParameterValue = @splat(ParameterValue{}),
    parameter_count: usize = 0,

    /// User locale
    locale_buffer: [16]u8 = @splat(0),
    locale_len: usize = 0,
...
pub const ResponseCard = struct {
    ...
    /// Card subtitle
    subtitle_buffer: [256]u8 = @splat(0),
    subtitle_len: usize = 0,

    /// Card body text
    body_buffer: [1024]u8 = @splat(0),
    body_len: usize = 0,

    /// Image URL
    image_url_buffer: [512]u8 = @splat(0),
    image_url_len: usize = 0,

    /// Action URL (deep link)
    action_url_buffer: [512]u8 = @splat(0),
...
pub const VoiceResponse = struct {
    ...
    speech_buffer: [2048]u8 = @splat(0),
    speech_len: usize = 0,

    /// Display text (if different from speech)
    display_text_buffer: [2048]u8 = @splat(0),
    ...
    reprompt_buffer: [512]u8 = @splat(0),
    reprompt_len: usize = 0,

    /// Disambiguation options (for needs_disambiguation)
    disambiguation_options: [4][128]u8 = @splat(@splat(0)),
    disambiguation_option_lens: [4]usize = @splat(0),
    disambiguation_count: usize = 0,

    /// Custom data to pass to app
    custom_data_buffer: [4096]u8 = @splat(0),
...
pub const VocabularyTerm = struct {
    /// The term/phrase
    term_buffer: [128]u8 = @splat(0),
    term_len: usize = 0,

    /// Alternative pronunciations
    pronunciations: [4][128]u8 = @splat(@splat(0)),
    pronunciation_lens: [4]usize = @splat(0),
