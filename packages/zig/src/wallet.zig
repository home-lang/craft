...
pub const Transaction = struct {
    ...
    pub fn withError(self: Transaction, code: []const u8, message: []const u8) Transaction {
        var txn = self;
        var err_code: [32]u8 = @splat(0);
        var err_msg: [128]u8 = @splat(0);
...
pub const WalletController = struct {
    ...
    pub fn init() WalletController {
        return .{
            ...
            .saved_cards_count = 0,
            .saved_cards = @splat(PaymentCard.init()),
            .region = [_]u8{ 'U', 'S' },
        };
    }
