...
pub const InvocationContext = struct {
    ...
    url_buffer: [2048]u8 = @splat(0),
    url_len: usize = 0,

    /// Query parameters from the URL
    query_params_buffer: [4096]u8 = @splat(0),
    ...
    referrer_buffer: [512]u8 = @splat(0),
    ...
    campaign_id_buffer: [128]u8 = @splat(0),
    campaign_id_len: usize = 0,

    /// User activity type for handoff
    activity_type_buffer: [256]u8 = @splat(0),
...
pub const MigrationData = struct {
    /// User preferences
    preferences_buffer: [4096]u8 = @splat(0),
    preferences_len: usize = 0,

    /// User account info (if signed in)
    account_id_buffer: [128]u8 = @splat(0),
    account_id_len: usize = 0,

    /// Cart/order data
    cart_data_buffer: [8192]u8 = @splat(0),
    cart_data_len: usize = 0,

    /// Progress data (for games/demos)
    progress_buffer: [4096]u8 = @splat(0),
    progress_len: usize = 0,

    /// Custom key-value data
    custom_data_buffer: [8192]u8 = @splat(0),
...
pub const CodeConfig = struct {
    ...
    fallback_url_buffer: [2048]u8 = @splat(0),
    fallback_url_len: usize = 0,

    /// Campaign identifier
    campaign_buffer: [128]u8 = @splat(0),
    campaign_len: usize = 0,

    /// Custom metadata
    metadata_buffer: [4096]u8 = @splat(0),
...
pub const AppClipCard = struct {
    ...
    title_buffer: [64]u8 = @splat(0),
    title_len: usize = 0,

    /// Subtitle
    subtitle_buffer: [128]u8 = @splat(0),
    subtitle_len: usize = 0,

    /// Action button text
    action_buffer: [32]u8 = @splat(0),
    action_len: usize = 0,

    /// Header image URL
    image_url_buffer: [2048]u8 = @splat(0),
