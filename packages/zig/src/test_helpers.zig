...
pub const TestHelpers = struct {
    ...
    pub fn isSidebarPaneVisible(split_view: macos.objc.id) bool {
        ...
        if (count_val < 2) {
            std.debug.print("[E2E Test] FAILED: Split view has less than 2 panes\n", .{});
            return false;
        }

        // Get the sidebar pane (first pane)
    ...
    /// Get comprehensive diagnostics for a split view
    pub fn dumpSplitViewDiagnostics(split_view: macos.objc.id, window: macos.objc.id) void {
        ...
        std.debug.print("[E2E Test] Number of panes: {d}\n", .{count_val});
        ...
        if (count_val >= 2) {
            ...
            std.debug.print("[E2E Test] Pane 0 (sidebar) frame: origin=({d:.1}, {d:.1}) size=({d:.1}x{d:.1})\n", .{
                pane0_frame.origin.x,
                pane0_frame.origin.y,
                pane0_frame.size.width,
                pane0_frame.size.height,
            });
            std.debug.print("[E2E Test] Pane 1 (content) frame: origin=({d:.1}, {d:.1}) size=({d:.1}x{d:.1})\n", .{
